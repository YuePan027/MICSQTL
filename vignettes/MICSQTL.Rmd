...
To conduct the analysis, initiation involves the use of a SummarizedExperiment 
object that contains bulk protein expression data (usually on log scale) in the `assays` slot. 
The row metadata (`rowData` slot) should contain information about the proteins. Additionally, incorporation of bulk gene expression data (usually on log scale) and a reference file (depending on the chosen method) as integral elements in the metadata slot is imperative. For more accurate cell-type fraction estimations, it's recommended 
to include marker genes only. 

This code chunk below demonstrates how to create a SummarizedExperiment object 
using a bulk protein matrix named "protein_data" and annotation information for 
proteins labeled "anno_protein". 
The gene expression data is stored in "gene_data." 
...
Further information is necessary for visualization and csQTL (cell-type specific quantitative trait loci) analysis. Additional metadata can be incorporated later 
using a command like `metadata(se)$new_data <- new_data` if needed. 
For detailed instructions, please refer to the following sections and function documentation.

In this package, we provide an example `SummarizedExperiment` object containing the following elements:
    
-   protein_data (assay): A subset of proteomic data PsychENCODE (on log scale) with 2,242 rows (protein) and 127 columns (sample).
...
-   ref_protein (in metadata): A signature matrix with 2,242 rows (protein) and 4 columns (cell types), which serves as a reference of known cellular signatures (on raw scale).

-   gene_data (in metadata): A data frame with 2,867 rows (genes) and 127 columns (sample) (on log scale).

-   ref_gene (in metadata): A signature matrix with 4,872 rows (genes) and 5 columns (cell types), which serves as a reference of known cellular signatures (on raw scale).
...
-   cell_counts (in metadata): A matrix containing cell counts across multiple subjects, where subjects are represented as rows and cell types as columns. Each entry 
...
The reference matrix for pure cell proteomics may be incomplete due to limitations inherent in single-cell proteomics technologies. To address this issue, we propose a novel cross-source cell-type fraction deconvolution method (Joint-AJ-RF) that capitalizes on matched bulk transcriptome-proteome data. In the following example, we illustrate the process of estimating protein proportions by integrating information from deconvoluted transcriptomes.

There are multiple options available for initializing sample-wise cellular fractions and purified proteomics, each with different input requirements. The following example illustrates the utilization of the CIBERSORT method for initial proportions (`pinit`), coupled with an external reference matrix (`ref_pnl`) containing the gene expression profiles or molecular signatures of different cell types. Typically, such references are obtained from small-scale single-cell or flow cytometry experiments. Please note that an internal log transformation is applied within the function, so `ref_pnl` should be provided on a raw scale if used.
