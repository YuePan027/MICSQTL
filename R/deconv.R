...
deconv <- function(se,
    ...
    if (!method %in% c("cibersort", "nnls")){
        stop("Only 'cibersort' or 'nnls' is valid in current version.")
    }
  ...
  if (length(in_use) < 50){
      warning("Less than 50 features are used to estimate cellular composition. The results may not be reliable.")
  }
