...
-   protein_data: A subset of proteomic data PsychENCODE (on log scale) with 2,242 rows (protein) and 127 columns (sample).
...
-   ref_protein: A signature matrix with 2,242 rows (protein) and 4 columns (cell types), which serves as a reference of known cellular signatures (on raw scale).

-   gene_data: A data frame with 2,867 rows (genes) and 127 columns (sample) (on log scale).

-   ref_gene: A signature matrix with 4,872 rows (genes) and 5 columns (cell types), which serves as a reference of known cellular signatures (on raw scale).
