...
ggplot(cbind(data.frame(reshape2::melt(slot(se, "metadata")$prop), MICSQTL::meta)), 
...
GGally::ggpairs(pca_res_protein, columns = seq_len(3), aes(color = disease, alpha = 0.5),
...
GGally::ggpairs(pca_res_gene, columns = seq_len(3), aes(color = disease, alpha = 0.5),
        upper = list(continuous = "points")) + theme_classic()
```
...
```{r filter2}
slot(se, "metadata")$SNP_data <- MICSQTL::SNP_data
slot(se, "metadata")$anno_SNP <- MICSQTL::anno_SNP
...
```{r plot3, echo  = FALSE}
ggplot(data.frame(reshape2::melt(data.frame(cbind(df_res, genotype = slot(se, "metadata")$SNP_data[idx,])), id.vars = c("genotype"))), 
