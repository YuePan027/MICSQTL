...
deconv <- function(se,
    ...
    if (source == "protein") {
        ...
        in_use <- intersect(rownames(assay(se)), 
                            rownames(slot(se, "metadata")$sig_protein))
    ...
    if (source == "transcript") {
        in_use <- intersect(
            rownames(slot(se, "metadata")$gene_data),
        ...
        gene_sub <- as.data.frame(slot(se, "metadata")$gene_data[in_use, , 
                                                                 drop = FALSE])
        ...
        if (method == "nnls") {
            decon_nnls <- apply(gene_sub, 2, function(y) 
                nnls::nnls(as.matrix(sig_gene), y)$x) %>% t()
            prop <- decon_nnls / rowSums(decon_nnls)
