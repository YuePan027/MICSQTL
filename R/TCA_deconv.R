#' Estimation of cellular expression in high-throughput data from heterogeneous
#'  tissues
#'
#' This function returns a `SummarizedExperiment` object including deconvoluted 
#' cellular sample-wise expression.
#'
...
TCA_deconv <- function(se, test = "array", prop) {
    if (test == "array") {
        ...
        res_tca <- TCA::tca(
            X = slot(se, "metadata")[[test]],
            W = prop,
            refit_W = FALSE,
            verbose = FALSE
        )
        res_full <- TCA::tensor(tca.mdl = res_tca, 
                                X = slot(se, "metadata")[[test]])
        names(res_full) <- colnames(prop)
