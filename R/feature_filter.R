...
#' The information from genetic variants should be stored in a P (the number of SNP) by N 
#' (the number of samples, should match the sample in `counts` slot) matrix contained as an element 
...
#' "POS" (position of that SNP) and "ID" (a unique identifier for each SNP, usually a combination of 
...
#' If "distance", then only cis-acting SNPs for each protein (defined as SNPs on the same chromosome and 
#' within 1M base pair (bp) range of that protein) will be included for downstream analysis.
#' if "null", then the same SNPs will be used for each protein.
#' @param filter_allele A numeric value denotes the threshold for minor allele frequency. Only works when `filter_method` 
#' contains "allele".
#' @param filter_geno A numeric value denotes the threshold for minimum genotype group proportion. 
...
#'
#' @return A `SummarizedExperiment`. The results after filtering will be stored as an element 
...
#' sig_gene = MICSQTL::ref_gene, gene_data = MICSQTL::gene_data, meta = MICSQTL::meta)
...
feature_filter <- function(se,
    ...
    if (!all(colnames(assay(se)) == colnames(slot(se, "metadata")$SNP_data))){
    stop("Samples in protein_data do not match that in SNP_data")
    }
    
    if (!nrow(slot(se, "metadata")$SNP_data) == nrow(slot(se, "metadata")$anno_SNP)){
    stop("SNPs contained in annotation data frame `anno_SNP` must match the SNPs in `SNP_data`")
    }
    
    if(!is.null(target_protein)){
    slot(se, "metadata")$target_dat <- assay(se)[target_protein, ]
    #se <- se[target_protein, ]
    } else {
    slot(se, "metadata")$target_dat <- assay(se)
    target_protein <- rownames(assay(se))
    }
    
    if(!is.null(target_SNP)){
    slot(se, "metadata")$SNP_data <- 
        slot(se, "metadata")$SNP_data[match(target_SNP, slot(se, "metadata")$anno_SNP$ID)
                                      , , drop=FALSE]
    slot(se, "metadata")$anno_SNP <- 
        slot(se, "metadata")$anno_SNP[match(target_SNP, slot(se, "metadata")$anno_SNP$ID)
                                      , , drop=FALSE]
    }
    
    #rowData(se) <- rowData(se)[match(rownames(assay(se)), rowData(se)$Symbol),]
    choose_SNP_list <- rep(list(seq_len(nrow(slot(se, "metadata")$SNP_data))), 
                           each = length(target_protein))
    
    if("allele" %in% filter_method){
    prop <- apply(slot(se, "metadata")$SNP_data, 1, prop_allele)
    prop2 <- apply(slot(se, "metadata")$SNP_data, 1, prop_geno)
    idx <- which(pmin(prop, 1-prop) > filter_allele & prop2 > filter_geno)
    slot(se, "metadata")$SNP_data <- slot(se, "metadata")$SNP_data[idx, , drop=FALSE]
    slot(se, "metadata")$anno_SNP <- slot(se, "metadata")$anno_SNP[idx, , drop=FALSE]
    choose_SNP_list <- rep(list(seq_len(nrow(slot(se, "metadata")$SNP_data))), 
                           each = length(target_protein))
    }
    if("distance" %in% filter_method){
    anno_SNP <- slot(se, "metadata")$anno_SNP
    choose_SNP_list <- bplapply(seq(len(length(target_protein))), function(i){
        message("Filter SNP based on distance for protein", target_protein[i], "\n")
    ...
    #se <- se[idx3,]
    slot(se, "metadata")$target_dat <- slot(se, "metadata")$target_dat[idx3,]
    choose_SNP_list <- choose_SNP_list[idx3]
    }
    
    slot(se, "metadata")$choose_SNP_list <- choose_SNP_list
    return(se)
}
