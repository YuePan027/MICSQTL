...
#' data(se)
#' se <- ajive_decomp(se, use_marker = TRUE)
#' cns_plot(se, score = "cns_1", group_var = "disease", scatter = TRUE, 
...
cns_plot <- function(se, score = "cns_1", group_var = "disease",
    ...
    p1 <- ggplot(
        df,
        aes(x = .data[[group_var]], y = .data[[score]], 
            fill = .data[[group_var]])
    ) +
        geom_point(
    ...
    p2 <- ggplot(
        df,
        aes(x = .data[[score]], y = .data[[group_var]], 
            fill = .data[[group_var]])
    ) +
        geom_density_ridges(
            aes(point_color = .data[[group_var]], 
    ...
    if (ncol(cns) < 2 | !scatter) {
        return(ggpubr::ggarrange(p1, p2, ncol = 2, nrow = 1, 
                                 common.legend = TRUE))
    } else if (scatter) {
        p3 <- ggplot(
            df,
            ...
            scale_point_color_hue(l = 40) +
            theme_classic()
        return(ggpubr::ggarrange(p1, p2, p3, ncol = 2, nrow = 2, 
                                 common.legend = TRUE))
    }
}
