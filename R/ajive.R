...
#'
#' This function returns a `SummarizedExperiment` object including results from AJIVE (Angle based Joint and Individual Variation Explained),
#' an integrative analysis tool and common normalized scored based on it.
#'
#' This implemented the AJIVE algorithm an integrative analysis method. 
#' It is useful when there are multiple data matrices measured on the same set of samples.
#' It decomposes each data matrix as three parts: (1) Joint variation across data types 
#' (2) Individual structured variation for each data type and (3) Residual noise. 
#' Common normalized scores are one of the desirable output to explore the joint behavior that is shared by different data sources.
#' 
#'
#' @param se A `SummarizedExperiment` object with bulk expression data frame contained in `counts` slot.
#' In addition, data measured on the same set of samples in `metadata` slot is required.
#' @param ini_rank A vector with each element corresponds to a initial signal rank for AJIVE decomposition. 
...
#'
#' @return A `SummarizedExperiment`. The results from AJIVE will be stored as an element (`ajive_res`) in `metadata` slot.
...
cns_plot <- function(se, score = "cns_1", group_var = "disease",
    ...
    p2 <- ggplot(df,
                 aes(x = .data[[score]], y = .data[[group_var]], fill = .data[[group_var]])) +
        geom_density_ridges(
            aes(point_color = .data[[group_var]], point_fill = .data[[group_var]], point_shape = .data[[group_var]]),
