#' Estimation of cellular composition in high-throughput data from heterogeneous 
#' tissues
#'
...
feature_filter <- function(se,
    ...
    if (!nrow(slot(se, "metadata")$SNP_data) == 
        nrow(slot(se, "metadata")$anno_SNP)) {
    ...
    if (!is.null(target_SNP)) {
        slot(se, "metadata")$SNP_data <-
            slot(se, "metadata")$SNP_data[
                match(target_SNP, slot(se, "metadata")$anno_SNP$ID),
                ,
                drop = FALSE
            ]
    ...
    if ("allele" %in% filter_method) {
        ...
        slot(se, "metadata")$SNP_data <- 
            slot(se, "metadata")$SNP_data[idx, , drop = FALSE]
        slot(se, "metadata")$anno_SNP <- 
            slot(se, "metadata")$anno_SNP[idx, , drop = FALSE]
        choose_SNP_list <- 
            rep(list(seq_len(nrow(slot(se, "metadata")$SNP_data))),
            each = length(target_protein)
        )
    }
    if ("distance" %in% filter_method) {
        ...
        choose_SNP_list <- bplapply(seq_len(length(target_protein)), 
                                    function(i) {
            message("Filter SNP based on distance for protein", 
                    target_protein[i], "\n")
        ...
        # se <- se[idx3,]
        slot(se, "metadata")$target_dat <- 
            slot(se, "metadata")$target_dat[idx3, ]
